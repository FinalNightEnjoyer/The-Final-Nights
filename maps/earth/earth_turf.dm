#define EARTH_GRASS_EDGE_LAYER  0.053
#define EARTH_VEGETATION_CHANCE 10 //1 in 10 chance of spawning vegetation/props
...
/turf/exterior/earth_grass
	name = "wild grass"
	icon = 'icons/turf/exterior/wildgrass.dmi'
	icon_edge_layer = EARTH_GRASS_EDGE_LAYER
	icon_has_corners = TRUE
	color = "#799c4b"
	footstep_type = /decl/footsteps/grass

/turf/exterior/earth_grass/Initialize()
	. = ..()
...
/turf/exterior/earth_grass/proc/generate_tile_prop()
	if(rand(0, EARTH_VEGETATION_CHANCE) != EARTH_VEGETATION_CHANCE || length(contents))
...
/turf/exterior/earth_grass/attackby(obj/item/W, mob/user, click_params)
...
/turf/exterior/earth_grass/fire_act(datum/gas_mixture/air, exposed_temperature, exposed_volume)
	if((temperature > T0C + 200 && prob(5)) || temperature > T0C + 1000)
		melt()

/turf/exterior/earth_grass/melt()
...
/turf/exterior/mud/dark/attackby(obj/item/W, mob/user)
	...
	if(istype(W, /obj/item/seeds))
		...
		if(S.seed_type == "grass" && user.do_skilled(5 SECONDS, SKILL_BOTANY, src) && !QDELETED(S) && user.unEquip(S))
			...
			qdel(S)
			ChangeTurf(/turf/exterior/earth_grass)
		return TRUE
	. = ..()
...
#undef EARTH_GRASS_EDGE_LAYER
#undef EARTH_VEGETATION_CHANCE